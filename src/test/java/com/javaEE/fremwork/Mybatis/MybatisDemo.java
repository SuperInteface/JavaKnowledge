package com.javaEE.fremwork.Mybatis;
...
import com.javaEE.fremwork.Mybatis.mapper.*;
...
import com.javaEE.fremwork.Mybatis.entity.EmailSexBean;
import com.javaEE.fremwork.Mybatis.entity.THealthReportFemale;
...
//		System.out.println(userTemp);
	...
	*/
	...
	@Test
	public void findAllByxxx(){
		SqlSession sqlsession = sqlSessionFactory.openSession();
		TUserMapper um = sqlsession.getMapper(TUserMapper.class);
		TUser u = new TUser();
		u.setUserName("lison");
		u.setRealName("王美丽");
		List<TUser> users = um.selectAllTest(u);
		System.out.println(users.toString());
	}

	@Test
	public void addUserForlist(){
		SqlSession sqlSession = sqlSessionFactory.openSession(true);
		TUserMapper um = sqlSession.getMapper(TUserMapper.class);
		TUser user1 = new TUser();
		user1.setUserName("A_zhangsan");
		user1.setRealName("A_张三");
		user1.setSex(Byte.valueOf("1"));
		user1.setMobile("10086");
		user1.setEmail("10086@163.com");
		user1.setNote("xxxxx");
		TPosition tp = new TPosition();
		tp.setId(2);
		user1.setPosition(tp);
		TUser user2 = new TUser();
		user2.setUserName("A_lisi");
		user2.setRealName("A_李四");
		user2.setSex(Byte.valueOf("1"));
		user2.setMobile("10086");
		user2.setEmail("10086@163.com");
		user2.setNote("xxxxx");
		user2.setPosition(tp);
		int i = um.insertForBatchA(Arrays.asList(user1, user2));
	...
	public void selectsss(){
		final TTest20201122Mapper mapper = sqlSessionFactory.openSession(true).getMapper(TTest20201122Mapper.class);
        Runnable runnable = new Runnable() {
            public void run() {
                System.out.println(mapper.selectAll().get(0).toString());
            }
        };
    }
